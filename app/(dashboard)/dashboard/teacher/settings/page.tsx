...
function TeacherSettingsSidebar({
  mobile = false,
  onNavigate,
  onLogout,
}: {
  ...
  return (
    <nav className={mobile ? "flex flex-col h-full" : ""}>
      ...
      <div className="p-3 flex-1">
        {NAV_ITEMS.map((item) => {
          const active = item.href === "/dashboard/teacher/settings"
          return (
            <Link
              key={item.href}
              href={item.href}
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex">
      ...
      <aside className="hidden lg:flex flex-col w-56 bg-white border-r border-gray-200 fixed inset-y-0">
        <TeacherSettingsSidebar onNavigate={() => setSidebarOpen(false)} onLogout={handleLogout} />
      </aside>
      ...
      {sidebarOpen && (
        <div className="lg:hidden fixed inset-0 z-40 flex">
          ...
          <div className="relative z-50 w-64 bg-white shadow-xl flex flex-col">
            ...
            <TeacherSettingsSidebar mobile onNavigate={() => setSidebarOpen(false)} onLogout={handleLogout} />
          </div>
        </div>
      )}
              ...
              {[
                {
                  ...
                  description: "When a school shortlists, interviews, or rejects your application",
                  value: notifApplicationUpdate,
                  setter: setNotifApplicationUpdate,
                },
                {
                  label: "New matching jobs",
                  description: "Jobs posted that match your subjects and teaching level",
                  value: notifNewJobs,
                  setter: setNotifNewJobs,
                },
                {
                  label: "School invitations",
                  description: "When a school invites you to apply for a role",
                  value: notifInvites,
                  setter: setNotifInvites,
                },
                {
                  label: "Newsletter & tips",
                  description: "Career advice, salary guides, and platform updates",
          ...
          <Section title="Danger Zone">
            <div className="space-y-4">
              <p className="text-sm text-gray-600">
                Deleting your account permanently removes your profile, applications, and quiz
