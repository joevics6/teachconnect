...
-- Used by the Subject Mastery percentile quiz.
-- difficulty_level = 'jss', is_active = true, no option nulls.
...
-- Same structure, slightly harder / curriculum-focused questions.
...
-- option_a/b/c/d and correct_option are left NULL — only question_text matters.
-- The written quiz sends only question_text to the teacher; AI grades the answer.
...
-- ─── Verify counts ───────────────────────────────────────────
-- Run this after inserting to confirm 30 rows were added:
-- SELECT difficulty_level, subject,
...
-- GROUP BY difficulty_level, subject;
